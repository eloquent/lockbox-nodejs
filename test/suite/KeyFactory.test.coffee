...
suite 'KeyFactory', =>
  ...
  setup =>
    ...
    @privateKeyStringPkcs8 =            """
                                        ...
                                        """
    @privateKeyStringPkcs8NoPassword =  """
                                        -----BEGIN PRIVATE KEY-----
  ...
  suite '#createPrivateKey()', =>
    ...
    test 'handles private keys in PKCS #8 format with passwords', =>
      key = @factory.createPrivateKey @privateKeyStringPkcs8, 'password'

      assert.strictEqual key.toPublicPem().toString(), @publicKeyString

    test 'handles private keys in PKCS #8 format without passwords', =>
      key = @factory.createPrivateKey @privateKeyStringPkcs8NoPassword
